...
use app\modules\modulemanagement\models\ModulesSearch;
...
class ModuleController extends Controller
{
    ...
    public function actionConfig($id)
    {
        ...
        $module = Yii::$app->getModule($model->module_name) ;

        $configModel = new $module->configModel ;

        if($configModel->load(Yii::$app->request->post())  && $configModel->validate() )
        {
            $model->config = json_encode($configModel->attributes) ;
            $model->save() ;
            return $this->redirect(['index']);
        }

        return $this->render('_'.$model->module_name.'config',['model'=>$configModel]);
